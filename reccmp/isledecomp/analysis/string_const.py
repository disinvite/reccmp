...
r_ascii = re.compile(rb"([\t\r\n\x20-\x7f]+)\x00")


def find_ascii_strings_in_buffer(
    buf: bytes, base_addr: int = 0
) -> Iterator[tuple[int, str]]:
    for match in r_ascii.finditer(buf):
        yield (base_addr + match.start(), match.group(1).decode("ascii"))


def find_ascii_strings(image: PEImage) -> Iterator[tuple[int, str]]:
