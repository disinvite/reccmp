...
class ListingOptions extends window.HTMLElement {
  ...
  onUpdate(state) {
    ...
    this.querySelector('input[type=search]').placeholder = state.filterType === 1
    ...
    this.querySelector('fieldset#pageDisplay > legend').textContent = `Page ${state.page + 1} of ${Math.max(1, state.pageCount())}`;
